...
export function PasswordGenerator() {
  ...
  const getLengthGradient = () => {
    ...
    if (length >= 10 && length < 13) color = "rgb(217, 119, 6)";
    if (length >= 13) color = "rgb(5, 150, 105)";
    return {
  ...
  const downloadExcel = () => {
    if (!sha256Hash || !bcryptHash) { alert("Generate a password first."); return; }
    const rows = [{ Timestamp: lastTimestamp || "", "SHA-256": sha256Hash, Bcrypt: bcryptHash }];
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Length slider — tour target */}
      <div data-tour="gen-length">
      ...
      {/* Character options checklist — tour target */}
      {password && (
        <div
          data-tour="gen-options"
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            ...
            <div className={`flex items-center gap-2 ${hasNumber ? "text-emerald-600" : "text-slate-400"}`}>
              <span>{hasNumber ? "✔" : "✖"}</span> Numbers (0–9)
            </div>
            <div className={`flex items-center gap-2 ${hasSymbol ? "text-emerald-600" : "text-slate-400"}`}>
              <span>{hasSymbol ? "✔" : "✖"}</span> Symbols (!@#)
            </div>
          </div>
        </div>
      )}

      {/* Generate button — tour target */}
      <button
        data-tour="gen-regenerate"
      ...
      {password && (
        <div className="space-y-3">
          ...
          {/* Password output field — tour target on the card */}
          <div data-tour="gen-output" className="bg-emerald-50 rounded-xl p-4 border-2 border-emerald-200">
            <div className="flex items-center justify-between mb-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            ...
            <div className="bg-slate-50 rounded-xl p-3 border-2 border-slate-200">
              ...
              <input type="text" value={bcryptHash} readOnly
